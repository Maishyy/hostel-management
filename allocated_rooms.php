...
if (isset($_POST['search'])) {
    ...
    // Retrieve manager_id and hostel_id based on username
    $query_manager = "SELECT manager_id, hostel_id FROM hostel_manager WHERE username = '$username'";
    $result_manager = mysqli_query($conn, $query_manager);
    $row_manager = mysqli_fetch_assoc($result_manager);
    $hostel_id = $row_manager['hostel_id'];

    // Select student details based on the search criteria and hostel_id
    $query_search = "SELECT a.*, s.Fname, s.Lname, r.Room_id
                     FROM Application AS a
                     INNER JOIN Student AS s ON a.Student_id = s.Student_id
                     INNER JOIN Room AS r ON a.Room_id = r.Room_id
                     WHERE a.Student_id LIKE '$search_box%' AND a.Hostel_id = '$hostel_id'";
    $result_search = mysqli_query($conn, $query_search);

    // Select the hostel name from the hostel table
    $query_hostel = "SELECT Hostel_name FROM Hostels WHERE Hostel_id = '$hostel_id'";
            ...
            <tbody>
                ...
                if (mysqli_num_rows($result_search) == 0) {
                    ...
                    while ($row_search = mysqli_fetch_assoc($result_search)) {
                        ...
                        $contact_number = $row_search['mob_no'];
                        $room_number = $row_search['Room_id'];

                        echo "<tr><td>{$student_name}</td><td>{$student_id}</td><td>{$contact_number}</td><td>{$hostel_name}</td><td>{$room_number}</td></tr>\n";
                    }
                }
                ?>
            </tbody>
...
//}
  ...
  /* if (isset($_POST['search'])) {
       ...
       $query6 = "SELECT * FROM Hostels WHERE Hostel_id = '$hostel_id'";
...
<?php
// Retrieve the manager ID and hostel ID based on the username
$username = $_SESSION['username'];
$sql = "SELECT manager_id, hostel_id FROM hostel_manager WHERE username = '$username'";
...
if ($result && mysqli_num_rows($result) > 0) {
    ...
    // Retrieve the student IDs from the application table for the selected hostel
    $query = "SELECT student_id ,room_id FROM application WHERE hostel_id = '$hostel_id'";
    ...
    if ($result && mysqli_num_rows($result) > 0) {
      ...
      //  echo '<th>Hostel</th>';
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            // Retrieve the student details based on the student ID
            $query2 = "SELECT * FROM student WHERE student_id = '$student_id'";
            $result2 = mysqli_query($conn, $query2);

            if ($result2 && mysqli_num_rows($result2) > 0) {
                while ($row2 = mysqli_fetch_assoc($result2)) {
                    $student_name = $row2['Fname'] . ' ' . $row2['Lname'];
                    $contact_number = $row2['Mob_no'];
                  //  $hostel = $row2['hostel'];
                    $room_id = $row['room_id'];
        ...
        echo 'No student data available for the selected hostel.';
    }
} else {
    echo 'Manager data not found.';
}
...
// retrieve manager_id based on username
$sql = "SELECT manager_id, hostel_id FROM hostel_manager WHERE username = '$username'";
$result = mysqli_query($conn, $sql);
...
if ($result && mysqli_num_rows($result) > 0) {
    ...
    // handle error: no matching manager found
    die("Error: could not retrieve manager_id");
}
